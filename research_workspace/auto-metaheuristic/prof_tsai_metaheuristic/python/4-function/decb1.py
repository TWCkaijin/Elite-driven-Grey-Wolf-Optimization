...
class decb1(de):
    ...
    def mutate(self, curr_pop):
        ...
        for i in range(self.pop_size):
            ...
            new_pop[i] = np.clip(curr_pop[i] + self.F * (self.best_sol - curr_pop[i]) + self.F * (s1 - s2), self.v_min, self.v_max)
        return new_pop
...
if __name__ == "__main__":
    ...
    elif ac() == 1: search = decb1()
    elif ac() == 11: search = decb1(si(1), si(2), si(3), ss(4), ss(5), si(6), sf(7), sf(8), sf(9), sf(10))
    else: usage()
