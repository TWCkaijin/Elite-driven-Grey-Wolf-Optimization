...
class deb2(de):
    ...
    def mutate(self, curr_pop):
        ...
        for i in range(self.pop_size):
            ...
            new_pop[i] = np.clip(self.best_sol + self.F * (s1 - s2) + self.F * (s3 - s4), self.v_min, self.v_max)
        return new_pop
...
if __name__ == "__main__":
    ...
    elif ac() == 1: search = deb2()
    elif ac() == 11: search = deb2(si(1), si(2), si(3), ss(4), ss(5), si(6), sf(7), sf(8), sf(9), sf(10))
    else: usage()
