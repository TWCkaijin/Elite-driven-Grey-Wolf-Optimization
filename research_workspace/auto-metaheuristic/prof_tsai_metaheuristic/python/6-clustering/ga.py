...
class ga:
    def __init__(self, num_runs = 3, num_evals = 1000, num_patterns_sol = 150, num_dims = 4, num_clusters = 3, filename_ini = "", filename_ins = "iris.data", pop_size = 20, cr = 0.6, mr = 0.1, num_players = 3, pra = 1):
    ...
    def run(self):
        ...
        for r in range(self.num_runs):
            ...
            while eval_count < self.num_evals:
                ...
                # 3. transition
                curr_pop = self.crossover(curr_pop, self.cr)
                curr_pop = self.mutate(curr_pop, self.mr)
                curr_pop = self.okm(curr_pop, centroids)
            avg_obj_val += self.best_obj_val
            if self.pra:
                self.redundant_analysis_2(curr_gen, r, pop_conv, final_state, avg_final_state, all_final_state)      # pra
    ...
    def okm(self, curr_pop, centroids):
        pop_size = len(curr_pop)
        for p in range(pop_size):
...
def usage():
    lib.usage("<#runs> <#evals> <#patterns> <#dims> <#clusters> <filename_ini> <filename_ins> <pop_size> <cr> <mr> <#players> <pra?>", common_params = "")

if __name__ == "__main__":
    if ac() == 2 and ss(1) == "-h": usage()
    if ac() == 1: search = ga()
    elif ac() == 13: search = ga(si(1), si(2), si(3), si(4), si(5), ss(6), ss(7), si(8), sf(9), sf(10), si(11), si(12))
    else: usage()
